const RuleCard = ({ rule, index }) => {
  return (
    <div className="relative group">
      {/* Glow effect on hover */}
      <div className={`absolute -inset-0.5 bg-gradient-to-r ${rule.gradient} rounded-2xl opacity-0 group-hover:opacity-20 blur-xl transition-opacity duration-500`} />
      
      <div className={`relative bg-gradient-to-br ${rule.bgGradient} backdrop-blur-sm rounded-2xl p-6 sm:p-7 md:p-8 border ${rule.borderColor} overflow-hidden transition-all duration-300 hover:border-opacity-60`}>
        {/* Subtle shine effect */}
        <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
        
        {/* Icon con gradiente */}
        <div className="relative mb-5 sm:mb-6">
          <div className="relative inline-flex">
            {/* Glow background */}
            <div className={`absolute inset-0 bg-gradient-to-r ${rule.gradient} rounded-xl blur-md opacity-40`} />
            
            {/* Icon container */}
            <div className={`relative bg-gradient-to-br ${rule.iconBg} backdrop-blur-sm p-3 sm:p-4 rounded-xl border ${rule.borderColor}`}>
              <rule.icon className="w-7 h-7 sm:w-8 sm:h-8 text-white" strokeWidth={2.5} />
            </div>
          </div>
        </div>
        
        {/* Content */}
        <div className="relative space-y-2 sm:space-y-3">
          <h3 className="text-xl sm:text-2xl font-black text-white leading-tight">
            {rule.title}
          </h3>
          
          <div className={`inline-flex px-3 py-1 bg-gradient-to-r ${rule.gradient} rounded-full`}>
            <p className="text-xs sm:text-sm font-bold text-white uppercase tracking-wide">
              {rule.subtitle}
            </p>
          </div>
          
          <p className="text-sm sm:text-base text-gray-300 leading-relaxed pt-2">
            {rule.description}
          </p>
        </div>

        {/* Decorative corner accent */}
        <div className={`absolute top-0 right-0 w-20 h-20 bg-gradient-to-br ${rule.gradient} opacity-10 blur-2xl rounded-full -translate-y-1/2 translate-x-1/2`} />
      </div>
    </div>
  );
};

export default RuleCard;
